...
<script>
	export default {
		...
		watch: { //值不是一开始出来的，在输入框输入时才能搜索到。
			message(newVal) { //获取了input里面的值
				var that = this;
				var cityId = this.$store.state.city.id; //城市id
				this.cancelRequest();
				
				//console.log(newVal); 快速输入时要把之前的去掉
				this.axios.get('/api/searchList?cityId='+cityId+'&kw=' + newVal, {
					cancelToken: new this.axios.CancelToken(function executor(c) {  //取消的方法，多次请求时在请求触发前取消
						that.source = c;
					})
				}).then((res) => {  //Vue axios请求频繁时取消上一次请求
