...
<script>
	...
	export default {
		...
		activated(){
			...
			this.isLoading = true;
			//console.log(123);  只有当城市id改变时才执行
			this.axios.get('/api/movieOnInfoList?cityId='+cityId).then((res) => {
				var msg = res.data.msg;
				if(msg === 'ok'){
					this.movieList = res.data.data.movieList;
					this.isLoading = false;
					this.prevCityId = cityId; //请求成功后的赋值
					/* this.$nextTick(() =>{
						var scroll = new BScroll(this.$reds.movie_body,{
							tap :true ,
		...
		methods:{
			handleToDetail(movieId){
				//console.log(movieId);
				this.$router.push('/movie/detail/' + movieId);  //跳转到电影详情页
			},
			handleToScroll(pos){
				if(pos.y > 30){
					this.pullDownMsg = '正在更新中...';
